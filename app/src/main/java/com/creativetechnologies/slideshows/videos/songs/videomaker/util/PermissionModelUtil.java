...
public class PermissionModelUtil {
    ...
    public boolean needPermissionCheck() {
        if (VERSION.SDK_INT >= 23 && this.context.checkSelfPermission("android.permission.WRITE_EXTERNAL_STORAGE") != PackageManager.PERMISSION_GRANTED) {
            return true;
        }
        return false;
    }
    ...
    public void showPermissionExplanationThenAuthorization() {
       // new Builder(this.context, android.R.style.TextAppearance_Theme_Dialog).setTitle(R.string.permission_check_title).setMessage(R.string.permission_check_message).setPositiveButton(R.string.ok, new C06441()).setCancelable(false).create().show();
        PermissionModelUtil.this.requestPermissions();
        PermissionModelUtil.this.sharedPrefs.edit().putBoolean(PermissionModelUtil.PERMISSION_CHECK_PREF, false).commit();
    }
