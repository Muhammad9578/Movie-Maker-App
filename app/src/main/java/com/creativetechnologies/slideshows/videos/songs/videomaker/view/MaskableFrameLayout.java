...
public class MaskableFrameLayout extends FrameLayout {
    ...
    @Nullable
    public static Bitmap mFinalMask = null;
    @Nullable
    ...
    public void swapBitmapMask(@Nullable Bitmap newMask) {
        if (newMask != null) {
            if (mFinalMask != null && !mFinalMask.isRecycled()) {
                mFinalMask.recycle();
            }
            mFinalMask = newMask;
            newBitmap = mFinalMask;
            Log.e("mFinalMask", "swapBitmapMask" + newBitmap);
        }
    }

    //Utils
    public PorterDuffXfermode getModeFromInteger(int index) {
